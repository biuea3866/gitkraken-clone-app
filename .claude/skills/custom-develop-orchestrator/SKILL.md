...
## 여러 티켓을 함께 돌릴 때 — 배치로 끊지 않는다

티켓 워크트리는 서로 독립이므로 **wave 의 티켓 전부를 한 번에 실행한다.** 4개씩 끊어 돌리면
노드 시간은 그대로인데 벽시계만 배수로 늘어난다 (wave 3 스펙 단계에서 45분 → 15분 차이였다).

- **[[custom-ticket-wave]] 가 이 일을 한다** — 티켓 의존을 위상정렬해 wave 단위로 러너를
  전량 동시에 스폰하고, 티켓별 워크트리와 소유 경로 교집합까지 실행 전에 검증한다.
  손으로 프로세스를 띄우는 것보다 그쪽을 쓴다 (배치 끊기가 구조적으로 불가능하다).
- 손으로 돌릴 때는 티켓 하나당 러너 프로세스 하나를 띄우고, 프로세스 안의 `--max-parallel` 은
  그 워크플로우의 wave 폭에 맞춘다 (5축이면 5, 6축이면 6).
- 게이트도 **티켓별이 아니라 wave 단위로 묶어** 한 번에 검토한다.
- 벤더 소진으로 실패하면 어댑터의 `[failover]` 가 1회 대체한다 — 폭을 줄이는 것보다 그쪽이 낫다.
...
- `remaining_blocking` 이 비어야 커밋·PR 로 간다. 남으면 고치고 **묻지 말고 다시 ⑧을 돌린다**
  (상한 3회). 라운드마다 사람에게 확인을 받으면 그 왕복이 실행 시간보다 길어진다 —
  게이트에서만 멈춘다는 원칙이 여기에도 적용된다. 3회를 넘기면 그때 보고한다.
- 재검증이 **같은 원인의 변형**을 계속 새로 낸다면 라운드를 더 돌리지 말고 멈춘다 —
  축 프롬프트의 클래스 스윕이 동작하지 않는다는 신호이므로 사람이 원인을 직접 본다.
