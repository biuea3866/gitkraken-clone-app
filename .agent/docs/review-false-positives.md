...
12. **(FP-A12) 기존 다수가 따르는 관례를 신규 한 줄에만 AC 위반으로 지적 금지.** 새 줄이
   **같은 선언 묶음의 기존 항목들과 동일한 방식**을 따랐다면, 그 방식 자체가 문제라도 지적 대상은
   그 줄이 아니라 **관례를 바꾸는 별도 티켓**이다. 신규 한 줄만 다르게 고치면 일관성이 깨지고,
   전부 고치면 티켓 범위를 넘는다. 판별법: 같은 묶음(enum 항목·목록 원소·표 행)에서 기존 항목
   다수가 같은 형태인지 확인한다. 그때는 finding 이 아니라 **게이트 보고**로 올린다.
   (근거: UND-47 이 `AppDestination.PATCH("Patch", …)` 를 추가했는데 기존 7개 목적지가 전부
   같은 하드코딩 라벨인데도 신규 한 줄만 p1 로 올라왔다. `"Patch"` 는 영어라 로케일 영향도 없었다.)
   **단 그 관례가 실제 결함을 낳고 있으면 게이트 보고에 그 사실을 함께 적는다** — 같은 관례
   (`strings: Strings = systemStrings()`)가 화면이 앱 언어 설정을 무시하는 결함이었고,
   별도 티켓(#125)으로 정리했다.
