...
**조작 전 대상 브랜치 위치**(`previousTarget`, UND-72 가 추가)를 담아 주고, `Succeeded` 는
**조작 직후 `RepositoryBaseline`**(UND-73 이 추가)도 함께 준다. `moveBranch` · `moveTag` ·
`hardResetBranch` 도 각자 변경 직후 캡처한 baseline 을 반환한다. `previousTarget` 은
