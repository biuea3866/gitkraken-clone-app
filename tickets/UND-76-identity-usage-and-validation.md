...
현재 계약에 **수정이 없다.** `saveProfile` 은 같은 이름을 거부하고, `deleteProfile` 후 `saveProfile`
조합은 그 사이 실패하면 **프로필을 잃는다** — 데이터 유실 경로다.

같은 이름을 유지한 채 이메일·서명 키를 바꾸는 **원자적 수정**을 계약에 넣거나, 수정 UX 를 "새 이름의
프로필 생성" 으로 제한한다고 명시한다. 어느 쪽이든 **delete + save 조합을 호출부가 쓰게 두지 않는다.**
