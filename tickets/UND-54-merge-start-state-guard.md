...
**UND-21 이 남긴 자기 결함이다** (기존 코드 정리가 아니다). UND-21 은 `infrastructure/git/merge/` 를
신설하면서 파괴적 연산(`abort`·`skip`)에 확인 토큰 + 실행 임계구역 내 상태 가드를 세웠지만,
같은 가드를 **시작 경로**(`merge`·`rebase`)에는 적용하지 않은 채 머지됐다. 재검증이 두 라운드에서
p1 으로 지적한 항목이며, 검증 라운드 상한을 적용해 이 티켓으로 미룬 것이다.
...
- UND-21 (머지 완료 — 이 티켓은 그 결과물의 남은 구멍을 닫는다)
