...
언어 목록은 UND-49 가 가진 카탈로그에서 얻는다. 목록에 없는 태그가 저장돼 있으면 **카탈로그 기본
로케일**로 표시하되 저장값은 지우지 않는다 (사용자가 나중에 그 로케일을 추가할 수 있다).

> 폴백 대상이 시스템 로케일이 아니라 카탈로그 기본값인 것은 `StringCatalog.resolveLocale` 의 기존
> 계약이다. 이 탭은 그 규칙을 바꾸지 않는다 — 바꾸려면 UND-49 소유 파일을 건드려야 한다.
...
- 카탈로그에 없는 언어 태그가 저장돼 있으면 **카탈로그 기본 로케일**로 표시하되 저장값은 유지된다
