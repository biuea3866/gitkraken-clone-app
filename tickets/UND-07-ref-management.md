...
## 작업 내용 (설계 의도)
`RefGateway` 를 구현한다. 로컬/원격 브랜치의 목록 조회·생성·이름 변경·삭제·체크아웃과
**태그의 목록 조회·생성·삭제**다.

**태그 이름 변경은 제공하지 않는다** — git 에서 태그 rename 은 삭제+재생성이라 이미 push 된
태그에서는 위험하다. `createTag(name, at, message)` 는 `message` 가 null 이면 lightweight,
있으면 annotated 태그다.
