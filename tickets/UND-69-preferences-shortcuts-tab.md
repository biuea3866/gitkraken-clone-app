...
- 재지정 · 항목별 기본값 복원. **"단축키 없음" 상태는 두지 않는다** — 저장 계약(`커맨드 id → 단축키`,
  값 non-null)에 그 표현이 없고, 항목을 지우는 것은 "기본값 복귀" 지 "없음" 이 아니다
...
- 오버라이드를 지우면 그 명령은 기본 단축키로 돌아가고, 다른 명령의 오버라이드는 영향받지 않는다
