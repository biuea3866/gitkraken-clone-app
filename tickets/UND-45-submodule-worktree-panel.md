...
현재 앱이 열고 있는 worktree 를 구분 표시한다. 다른 worktree 는 **열기 요청을 콜백으로 노출**한다 — 탭으로 여는 실제 배선은 통합 티켓(UND-51)이 한다.
UND-44 와 같은 wave 라 탭 API 를 전제할 수 없기 때문이며, 이렇게 두면 두 티켓을 병렬로 진행할 수 있다.

제거는 UND-34 가 더티 여부를 판정해 주므로, 더티하면 **무엇이 유실되는지 파일 수와 함께** 경고한다.

**롤백**: 서브모듈 초기화·업데이트는 이전 커밋으로 재지정해 되돌린다 — worktree 제거는 되돌릴 수 없으므로 더티 시 유실될 파일 수를 경고한다.
...
- 서브모듈·worktree 열기 요청이 콜백으로 노출된다 (탭 배선은 UND-51 소관)
