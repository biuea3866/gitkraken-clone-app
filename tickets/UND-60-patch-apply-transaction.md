...
적용 결과는 `ApplyOutcome` 하나로 돌려준다. **3-way 적용이 필요한 패치는
`Unsupported(THREE_WAY_REQUIRED)`** 로 보고하고 워킹트리에 **충돌 표식을 남기지 않는다** —
JGit 표준 API 에 조상 blob 기반 3-way 경로가 없다. 충돌은 `Conflicted(paths)` 로 경로만 알린다.
어느 경우에도 워킹트리는 무변경이므로 UND-23 충돌 에디터가 이어받을 대상이 생기지 않는다.
...
- 되돌릴 수 없는 상태는 트리에 담기지 않으므로 적용 전에 **거부**한다 — 담을 수 없는 것을
  담은 척하지 않는다. 판정은 종류를 열거하지 않고 **"이 상태를 트리에 담았다가 그대로 되돌릴 수
  있는가"** 하나로 묻는다. 여기에 걸리는 것: **추적되지 않은 파일**, 그리고 **파일을 하나도 품지
  않은 추적되지 않은 디렉터리**(적용 대상 자리 · 그 조상 · 대상 아래). 대상 자리 아래는 하위까지
  훑고 조상은 자기 자신만 본다 — 조상 아래까지 훑으면 사라지지도 않을 형제 빈 디렉터리까지
  거부하는 과잉이 된다.

> **JGit 으로 확정했다.** 패치 적용과 격리 검증 모두 JGit 으로 하고 **외부 `git` 바이너리를
> 호출하지 않는다** — 루트 `CLAUDE.md` 의 JGit 순수 구현 전제를 따른다. 미설치 동작·정리 정책을
> 정할 필요가 없다(외부 프로세스 경로 자체가 없다).
...
- 경로 이탈이 포함된 패치는 거부된다
- 3-way 가 필요한 패치는 워킹트리 무변경으로 `Unsupported(THREE_WAY_REQUIRED)` 를 돌려준다
...
- **파일 없는 추적되지 않은 디렉터리가 적용 대상 자리·조상·대상 아래에 있으면 적용을 거부한다**
- **거부된 경우 워킹트리가 무변경으로 보존된다**
