...
class StringCatalogSpec : FunSpec({
    ...
    test("구현 전 화면의 빈 네임스페이스 스텁은 병합에서 아무 키도 더하지 않는다") {
        // UND-45는 자기 스텁을 채웠고, 나머지 여섯 화면만 아직 빈 맵이다.
        val emptyStubs = builtInTranslations.filter { it.isEmpty() }
        emptyStubs.size shouldBe WAVE8_STUB_NAMESPACES.size - 1

        mergeTranslations(emptyStubs) shouldBe emptyMap()
