 ...
 * value class·로케일별 번역은 UND-40(설정 화면)이 **이 파일 안에서만** 채운다. 공통 파일
 * (`BuiltInStrings.kt`)은 이미 등록돼 있으므로 건드리지 않는다 — 같은 wave 의 화면 7건이 그 파일을
 * 함께 고치면 머지 충돌이 난다.
 *
 * 채우는 모양은 [CommonStrings] 가 정본이다: [PREFERENCES_NAMESPACE] 로 키를 만들고, 번역 맵을
 * 로케일별로 채우고, `Strings.preferences` 확장 프로퍼티로 노출한다.
 *
...
/** 환경설정 화면이 추가할 `preferences.*` 키의 자리. */
object PreferencesKeys

/** 환경설정 문구 접근자. UND-40 이 여기에 화면별 문자열을 추가한다. */
@JvmInline
...
internal val preferencesTranslations: Map<Locale, Map<StringKey, String>> = emptyMap()
